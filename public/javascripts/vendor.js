...
_ref = require("../../server/models"), Shot = _ref.Shot, Station = _ref.Station;

shotFactory = require("../factories/shotFactory");

stationFactory = require("../factories/stationFactory");

propertiesToCheck = ["_id", "image", "thumbnail", "created", "printed"];

module.exports = function(app) {
  return describe("shots resource", function() {
    return describe("GET /api/shots", function() {
      before(function(done) {
        var _this = this;
        this.startDate = moment();
        this.endDate = moment(this.startDate).add('days', 1);
        this.numberOfStations = _.random(2, 5);
        this.numberOfShots = _.random(10, 20);
        return Station.remove({}).exec(function(err) {
          var _i, _ref1, _results;
          return async.map((function() {
            _results = [];
            for (var _i = 1, _ref1 = _this.numberOfStations; 1 <= _ref1 ? _i <= _ref1 : _i >= _ref1; 1 <= _ref1 ? _i++ : _i--){ _results.push(_i); }
            return _results;
          }).apply(this), _.bind(stationFactory.generate, _this), function(err, stations) {
            if (err) {
              return done(err);
            }
            _this.stations = stations;
            _this.stationsJson = _.map(stations, function(s) {
              return JSON.parse(JSON.stringify(s));
            });
            return Shot.remove({}).exec(function(err) {
              var _j, _ref2, _results1;
              return async.map((function() {
                _results1 = [];
                for (var _j = 1, _ref2 = _this.numberOfShots; 1 <= _ref2 ? _j <= _ref2 : _j >= _ref2; 1 <= _ref2 ? _j++ : _j--){ _results1.push(_j); }
                return _results1;
              }).apply(this), _.bind(shotFactory.generate, _this), function(err, shots) {
                if (err) {
                  return done(err);
                }
                _this.shots = shots;
                _this.shotsJson = _.map(shots, function(s) {
                  return JSON.parse(JSON.stringify(s));
                });
                return done();
              });
      ...
      it("just returns some of the latest shots when nothing is passed", function(done) {
        var _this = this;
        return request(app).get("/api/shots").expect('Content-Type', 'application/json').expect(200).end(function(err, res) {
          var json, meta;
          if (err) {
            return done(err);
          }
          ...
          meta = res.body.meta;
          meta.should.have.enumerable('total', _this.shotsJson.length);
          meta.should.have.enumerable('count', json.shots.length);
          _.each(json.shots, function(shot) {
            var idx, lhs, rhs;
            idx = _.findIndex(_this.shotsJson, {
              "_id": shot._id
            });
            idx.should.not.eql(-1);
            lhs = _.pick(shot, propertiesToCheck);
            rhs = _.pick(_this.shotsJson[idx], propertiesToCheck);
            return lhs.should.eql(rhs);
          });
          return done();
        });
      });
      it("supports 'limit' query parameter", function(done) {
        var limit,
          _this = this;
        limit = _.random(1, this.numberOfShots);
        return request(app).get('/api/shots').query({
          limit: limit
          ...
          lhs = _(_this.shotsJson).sortBy(function(s) {
            return -moment(s.created).unix().first(limit).collect(function(s) {
              return _.pick(s, propertiesToCheck).value();
            });
          });
          rhs = _(res.body.shots).collect(function(s) {
            return _.pick(s, propertiesToCheck).value();
          });
          lhs.should.eql(rhs);
          return done();
      ...
      it("supports filtering by a single status", function(done) {
        var status,
          _this = this;
        ...
        return request(app).get('/api/shots').query({
          ...
          lhs = _(_this.shotsJson).sortBy(function(s) {
            return -moment(s.created).unix().filter(function(s) {
              return s.status === status.collect(function(s) {
                return _.pick(s, propertiesToCheck).value();
              });
            });
          });
      ...
      it("supports filtering by multiple statuses", function(done) {
        var statuses,
          _this = this;
        ...
        return request(app).get('/api/shots').query({
          ...
          lhs = _(_this.shotsJson).sortBy(function(s) {
            return -moment(s.created).unix().filter(function(s) {
              return _.contains(statuses, s.status).collect(function(s) {
                return _.pick(s, propertiesToCheck).value();
              });
            });
          });
      ...
      it("supports filtering by station", function(done) {
        var _this = this;
        return async.each(this.stationsJson, function(station, cb) {
          return request(app).get('/api/shots').query({
            printed_on: station._id
          }).expect('Content-Type', 'application/json').expect(200).end(function(err, res) {
            var lhs, rhs;
            lhs = _(_this.shotsJson).sortBy(function(s) {
              return -moment(s.created).unix().filter(function(s) {
                return (s.printedOn != null) && s.printedOn.toString() === station._id.collect(function(s) {
                  return _.pick(s, propertiesToCheck).value();
                });
              });
            });
      ...
      return it("supports pagination using 'max_timestamp' parameter", function(done) {
        var _this = this;
        return async.each([1, 2, 3, 4, 5, 6], function(i, cb) {
          var e, max_timestamp, s;
          s = _this.startDate.unix();
          e = _this.endDate.unix();
          ...
          return request(app).get('/api/shots').query({
            ...
            lhs = _(_this.shotsJson).sortBy(function(s) {
              return -moment(s.created).unix().filter(function(s) {
                return moment(s.created).unix() < max_timestamp.collect(function(s) {
                  return _.pick(s, propertiesToCheck).value();
                });
              });
            });
...
var Faker, Shot, availableStatuses, moment, uid, _;
...
Shot = require("../../server/models").Shot;

module.exports.availableStatuses = availableStatuses = ["initial", "queued", "printed", "failed"];

module.exports.generate = function(index, cb) {
  var e, payload, s, shot, _ref;
  payload = {
    hash: uid(24),
  ...
  if ((this.startDate != null) && (this.endDate != null)) {
    s = moment(this.startDate).unix();
    e = moment(this.endDate).unix();
    payload.created = moment.unix(s + _.random(0, e - s)).toDate();
  }
  if ((payload.status === "printed") && ((_ref = this.stations) != null ? _ref.length : void 0) > 0) {
    payload.printedOn = _.sample(this.stations);
  }
...
;require.register("test/factories/stationFactory", function(exports, require, module) {
var Faker, Station, moment, uid, _;
