...
var Shot, ShotCollection, _ref,
  __hasProp = {}.hasOwnProperty,
  __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };

Shot = require("models/shot");

module.exports = ShotCollection = (function(_super) {
  __extends(ShotCollection, _super);

  function ShotCollection() {
    _ref = ShotCollection.__super__.constructor.apply(this, arguments);
    return _ref;
  }

  _.extend(ShotCollection.prototype, Chaplin.SyncMachine);

  ShotCollection.prototype.model = Shot;

  ShotCollection.prototype.initialize = function() {
    ShotCollection.__super__.initialize.apply(this, arguments);
    this.on('request', this.beginSync);
    this.on('sync', this.finishSync);
    return this.on('error', this.unsync);
  };

  ShotCollection.prototype.parse = function(response, options) {
    this.meta = response.meta;
    return response.shots;
  };

  ShotCollection.prototype.url = "/api/shots";
...
module.exports = ShotsController = (function(_super) {
  ...
  ShotsController.prototype.index = function() {
    return this.view = new ShotGridView({
      region: "main"
    });
  };
...
module.exports = Shot = (function(_super) {
  ...
  _.extend(Shot.prototype, Chaplin.SyncMachine);

  Shot.prototype.initialize = function() {
    Shot.__super__.initialize.apply(this, arguments);
    this.on('request', this.beginSync);
    this.on('sync', this.finishSync);
    return this.on('error', this.unsync);
  };
...
buf.push("<div class=\"polaroid\"><div class=\"photo-area\"><a" + (jade.attr("href", jade.url('shots#show', {id : shot._id}), true, false)) + "><img" + (jade.attr("src", "" + ( shot.image ) + "", true, false)) + " class=\"polaroid-photo img-responsive\"/></a><div class=\"photo-info\">");
if ( jade.auth())
{
...
switch (shot.status){
case "failed":
buf.push("Не напечатана");
  break;
case "printed":
if ( shot.printedOn)
{
buf.push("<a" + (jade.attr("href", jade.url('stations#show', { name : shot.printedOn.name }), true, false)) + ">" + (jade.escape(null == (jade.interp = shot.printedOn.title) ? "" : jade.interp)) + "</a>");
}
  break;
case "queued":
buf.push("В очереди");
...
var CollectionView, ShotCollection, ShotGridItemView, ShotGridView, View, _ref,
  __hasProp = {}.hasOwnProperty,
  __extends = function(child, parent) { for (var key in parent) { if (__hasProp.call(parent, key)) child[key] = parent[key]; } function ctor() { this.constructor = child; } ctor.prototype = parent.prototype; child.prototype = new ctor(); child.__super__ = parent.prototype; return child; };
...
module.exports = ShotGridView = (function(_super) {
  ...
  _.extend(ShotGridView.prototype, Chaplin.EventBroker);
  ...
  ShotGridView.prototype.initialize = function() {
    ...
    this.subscribeEvent('window-scrolled-bottom', this.loadNextPortion);
    return this.loadNextPortion();
  };
  ...
  ShotGridView.prototype.loadNextPortion = function() {
    ...
    if (!this.collection.isEmpty()) {
      query.max_timestamp = _.min(this.collection.map(function(s) {
        return moment(s.attributes.created).unix();
      }));
    }
